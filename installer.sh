...
TMPPATH=/tmp/RadioGit-install
FILEPATH=/tmp/RadioGit-main.tar.gz

echo "Starting RadioGit installation..."

if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/RadioGit
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/RadioGit
fi
...
cleanup() {
    ...
    [ -d "/tmp/RadioGit-main" ] && rm -rf "/tmp/RadioGit-main"
}
...
echo "Downloading RadioGit..."
wget --no-check-certificate 'https://github.com/Belfagor2005/RadioGit/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "Failed to download RadioGit package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to extract RadioGit package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/RadioGit-main/usr/lib/enigma2/python/Plugins/Extensions/RadioGit" ]; then
    cp -r "$TMPPATH/RadioGit-main/usr/lib/enigma2/python/Plugins/Extensions/RadioGit"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/RadioGit-main/usr/lib64/enigma2/python/Plugins/Extensions/RadioGit" ]; then
    cp -r "$TMPPATH/RadioGit-main/usr/lib64/enigma2/python/Plugins/Extensions/RadioGit"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/RadioGit-main/usr" ]; then
    cp -r "$TMPPATH/RadioGit-main/usr"/* /usr/ 2>/dev/null
